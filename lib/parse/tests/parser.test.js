...
describe('Parses entire `lit`s', () => {
  ...
  it('Parses PR1', () => {
    const parsed = raw.course.PR1.reduce(parse.course, [])
    
    expect(data.course.PR1).toEqual(parsed)
  })

  it('Parses PR2', () => {
    const parsed = raw.course.PR2.reduce(parse.course, [])

    expect(data.course.PR2).toEqual(parsed)
  })

  it('Parses Q1', () => {
    const parsed = raw.course.Q1.reduce(parse.course, [])

    expect(data.course.Q1).toEqual(parsed)
  })

  it('Parses PR4', () => {
    const parsed = raw.course.PR4.reduce(parse.course, [])

    expect(data.course.PR4).toEqual(parsed)
  })

  it('Parses S1', () => {
    const parsed = raw.course.S1.reduce(parse.course, [])
...
describe('Parses history from 2013 to 2018', () => {
  ...
  it('Parses 2013', () => {
    const parsed = raw.history.reduce(parse.history, [])
